...
/**
 * Wrapper class to simplify cluster configuration. This is sample code and is
 * intended to show a mechanism for
 * managing the configuration of the components required for Aeron Cluster. This
 * code may change between versions
...
public final class ClusterConfig {
    ...
    /**
     * Create a new ClusterConfig. This call allows for 2 separate lists of
     * hostnames, so that there can be 'external'
     * addresses for ingress requests and 'internal' addresses that will handle all
     * the cluster replication and
     * control traffic.
     *
     * @param startingMemberId   id for the first member in the list of entries.
     * @param memberId           id for this node.
     * @param ingressHostnames   list of hostnames that will receive ingress request
     *                           traffic.
     * @param clusterHostnames   list of hostnames that will receive cluster
     *                           traffic.
     * @param portBase           base port to derive remaining ports from.
     * @param clusteredService   instance of the clustered service that will run
     *                           with this configuration.
     * @param additionalServices instances of additional clustered services that
     *                           will run with this configuration.
    ...
    public static ClusterConfig create(
        ...
        final String memDir = System.getenv("MEM_DIR") + "/aeron-cluster-" + memberId;
        // System.out.println("MEM_DIR: " + System.getenv("MEM_DIR"));
        // System.out.println("ARCHIVE_DIR: " + System.getenv("ARCHIVE_DIR"));
        // System.out.println("DISK_DIR: " + System.getenv("DISK_DIR"));

        final String archiveDir = System.getenv("ARCHIVE_DIR") + "/aeron-cluster-" + memberId;
        final String diskDir = System.getenv("DISK_DIR") + "/aeron-cluster-" + memberId;
        ...
        final MediaDriver.Context mediaDriverContext = new MediaDriver.Context()
                .aeronDirectoryName(memDir)
                .threadingMode(ThreadingMode.SHARED)
                .termBufferSparseFile(true)
                .mtuLength(8192)
                .ipcMtuLength(8192)
                .multicastFlowControlSupplier(new MinMulticastFlowControlSupplier());
        // System.out.println("mediaDriverContext: " + mediaDriverContext);

        final AeronArchive.Context replicationArchiveContext = new AeronArchive.Context()
                .controlResponseChannel(udpChannel(memberId, hostname, portBase, 0));

        final String replicationChannel = udpChannel(memberId, hostname, portBase, 0);

        final Archive.Context archiveContext = new Archive.Context()
                .aeronDirectoryName(memDir)
                .archiveDir(new File(archiveDir))
                .markFileDir(new File(diskDir))
                .controlChannel(udpChannel(memberId, hostname, portBase, ARCHIVE_CONTROL_PORT_OFFSET))
                .archiveClientContext(replicationArchiveContext)
                .localControlChannel("aeron:ipc?term-length=128k|alias=service-control")
        ...
        final AeronArchive.Context aeronArchiveContext = new AeronArchive.Context()
                ...
                .aeronDirectoryName(memDir);
        // System.out.println("archiveContext: " + archiveContext);
        ...
        final ConsensusModule.Context consensusModuleContext = new ConsensusModule.Context()
                ...
                .clusterId(0)
                .markFileDir(new File(diskDir))
                .wheelTickResolutionNs(4000000)
                .ticksPerWheel(8);
        // System.out.println("consensusModule: " + consensusModuleContext);
     ...
     *
     * @param baseDir parent directory to be used for archive and cluster stored
     *                data.
     */
     ...
     *
     * @return configured
     *         {@link io.aeron.cluster.service.ClusteredServiceContainer.Context}.
     ...
     * @param clusterHostnames of the cluster members internal address (can be the
     *                         same as 'hostnames').
     * @param portBase         initial port to derive other port from via
     *                         appropriate node id and offset.
     * @return the String which can be used for
     *         {@link io.aeron.cluster.ClusterMember#parse(String)}.
     */
     ...
     * {@link io.aeron.cluster.ClusterMember#parse(String)}.
     *
     * @param startingMemberId first memberId to be used in the list of
     *                         clusterMembers. The memberId will increment by 1
     ...
     * Ingress endpoints generated from a list of hostnames.
     *
     * @param startingMemberId       first memberId to be used when generating the
     *                               ports.
     * @param hostnames              for the cluster members.
     * @param portBase               Base port for the cluster
     * @param clientFacingPortOffset Offset for the client facing port
    ...
    /**
     * Calculates a port for use with a node based on a specific offset. Can be used
     * with the predefined offsets, e.g.
     * {@link ClusterConfig#ARCHIVE_CONTROL_PORT_OFFSET} or with custom offsets. For
     * custom offsets select a value
     * larger than largest predefined offsets. A value larger than the largest
     * predefined offset, but less than
